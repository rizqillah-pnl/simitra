...
if (isset($_POST['daftarLowongan'])) {
    ...
    $cari = mysqli_query($conn, "SELECT * FROM tb_lowongan_user WHERE id='$idLowongan' AND id_user='$kode'");
    if (mysqli_num_rows($cari) == 0) {

        date_default_timezone_set('Asia/Jakarta');
        $now = date("Y-m-d H-i-s");
        $daftar = mysqli_query($conn, "INSERT INTO tb_lowongan_user (id_lowongan, id_petugas, tanggal_daftar) VALUES ('$idLowongan', '$kode', '$now')");
